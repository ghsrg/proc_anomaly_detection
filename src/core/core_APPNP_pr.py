...
class APPNP_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None, K=10, alpha=0.1):
        super().__init__()
    ...
    def forward(self, data_batch, doc_features=None):
        x = self.activation(self.lin(data_batch.x))
        x = self.appnp(x, data_batch.edge_index)
        x_pooled = self.global_pool(x, data_batch.batch)

        if doc_features is not None:
            doc_emb = self.activation(self.doc_fc(doc_features.to(x.device)))
        else:
            doc_emb = torch.zeros((x_pooled.size(0), self.doc_fc.out_features), device=x.device)
