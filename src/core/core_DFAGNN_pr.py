...
class DFAGNN_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None, num_subspaces=3):
        ...
        self.sub_gnns = nn.ModuleList([
            GCNConv(input_dim, hidden_dim) for _ in range(num_subspaces)
        ])
        self.activation = nn.ReLU()
        self.global_pool = global_mean_pool

        self.doc_fc = nn.Linear(doc_dim, hidden_dim * num_subspaces)

        self.bnf = nn.LayerNorm(hidden_dim * num_subspaces * 2)
        self.fusion_head = nn.Linear(hidden_dim * num_subspaces * 2, hidden_dim)
    ...
    def forward(self, data_batch, doc_features=None):
        ...
        # Кожна GCN обробляє ті самі features, але вважається окремою проєкцією-перспективою
        sub_outputs = [
            self.activation(gnn(x.to(device), edge_index.to(device)))
            for gnn in self.sub_gnns
        ]

        pooled_subs = [
            self.global_pool(h, batch.to(device)) for h in sub_outputs
        ]

        x_cat = torch.cat(pooled_subs, dim=1)  # [batch, hidden_dim * num_subspaces]

        if doc_features is not None:
            doc_emb = self.activation(self.doc_fc(doc_features.to(device)))
        else:
            doc_emb = torch.zeros((x_cat.size(0), self.doc_fc.out_features), device=device)

        x = torch.cat([x_cat, doc_emb], dim=1)
        x = self.bnf(x)
