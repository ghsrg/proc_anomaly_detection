...
class GraphMixer_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None):
        ...
        # Token-mixing Transformer (вузли → один embedding)
    ...
    def forward(self, data):
        ...
        # Збирання у послідовності
        unique_batches = batch.unique()
        x_seq = [x[batch == b] for b in unique_batches]
        max_len = max(t.size(0) for t in x_seq)
        x_padded = torch.stack([
            torch.cat([t, t.new_zeros(max_len - t.size(0), t.size(1))])
            for t in x_seq
        ])
        x_mixed = self.token_mixer(x_padded)[:, 0]  # CLS токен

        # Документ
        if doc_features is not None:
            doc_emb = self.activation(self.doc_proj(doc_features))
        else:
            doc_emb = torch.zeros(x_mixed.shape[0], self.doc_proj.out_features, device=device)

        x = torch.cat([x_mixed, doc_emb], dim=1)
        x = self.bnf(x)
