...
class MuseGNN_old_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None):
        ...
        # Документ
        self.doc_fc = nn.Linear(doc_dim, hidden_dim * 3)

        # Злиття
        self.bnf = nn.LayerNorm(hidden_dim * 6)
        self.fusion_head = nn.Linear(hidden_dim * 6, hidden_dim)
    ...
    def forward(self, data_batch, doc_features=None):
        ...
        x = torch.cat([x_cat, doc_emb], dim=1)  # [batch, hidden_dim*6]
        x = self.bnf(x)
