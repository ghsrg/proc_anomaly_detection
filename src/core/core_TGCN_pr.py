...
class TGCN_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None):
        super(TGCN_pr, self).__init__()
        print(f"📦 Init TGCN_pr with input_dim={input_dim}, output_dim={output_dim}")
    ...
    def forward(self, sequence, doc_features=None):
        """
        :param sequence: List[Data] (послідовність кроків процесу)
        :param doc_features: тензор [seq_len, doc_dim]
        """
        ...
        output_seq, _ = self.gru(h_seq)     # [1, seq_len, hidden_dim]
        h_final = output_seq[0, -1, :]      # останній крок
