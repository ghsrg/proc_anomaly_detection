...
class TemporalGAT_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None):
        super().__init__()
        print(f"📦 Init TemporalGAT_pr with input_dim={input_dim}, output_dim={output_dim}")

        self.time_embedding = nn.Linear(1, 1)  # перетворює delta_t в ембедінг (можна замінити на sin/cos)
        self.gat1 = GATConv(input_dim + 1, hidden_dim, heads=2, concat=False)
    ...
    def forward(self, data):
        ...
        # Обчислення delta_t як різниці до поточного моменту
        if timestamps is not None:
            timestamps = timestamps.to(device)
            max_time = timestamps.max()
            delta_t = max_time - timestamps.view(-1, 1)
            t_encoded = self.time_embedding(delta_t)
            x = torch.cat([x, t_encoded], dim=1)
        else:
            # Якщо немає timestamp — додаємо нулі
            x = torch.cat([x, torch.zeros(x.size(0), 1, device=device)], dim=1)
        ...
        if doc_features is not None:
            ...
            doc_emb = torch.zeros(x.size(0), self.doc_fc.out_features, device=device)
