...
class TransformerMLP_pr(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, doc_dim, edge_dim=None, num_nodes=None):
        super(TransformerMLP_pr, self).__init__()

        self.input_fc = nn.Linear(input_dim, hidden_dim)
        self.pos_encoder = nn.Embedding(500, hidden_dim)  # позиційне кодування до 500 кроків
    ...
    def forward(self, data):
        ...
        if timestamps is not None:
            pos_ids = torch.clamp(timestamps.long(), max=self.pos_encoder.num_embeddings - 1)
            pos_emb = self.pos_encoder(pos_ids)
        else:
            pos_emb = torch.zeros_like(x)

        x = self.activation(self.input_fc(x)) + pos_emb
        if batch is None:
            batch = torch.zeros(x.size(0), dtype=torch.long, device=x.device)

        # Збір у [batch_size, seq_len, hidden_dim]
        unique_batches = batch.unique()
        x_seq = []
        for b in unique_batches:
            x_seq.append(x[batch == b])
        max_len = max([t.size(0) for t in x_seq])
        x_padded = torch.stack([
            torch.cat([t, t.new_zeros(max_len - t.size(0), t.size(1))])
            for t in x_seq
        ])
        x_transformed = self.transformer(x_padded)[:, 0]  # [CLS] токен-еквівалент

        if doc_features is not None:
            doc_emb = self.activation(self.doc_fc(doc_features))
        else:
            doc_emb = torch.zeros(x_transformed.shape[0], self.doc_fc.out_features, device=x.device)

        fusion = torch.cat([x_transformed, doc_emb], dim=1)
